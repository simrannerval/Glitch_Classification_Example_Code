...
<p align="center">
  ...
  <br />
  <strong>Figure 2:</strong> <em> Left: Flow chart showing how the ACT pipeline goes from raw data to maps. The portions we have modified are shown in purple, and the new glitch classification pipeline is shown in teal. Right: Half-degree cut-outs of ACT depth-1 maps with a simulated source with an amplitude of 12,000mJy and a 5000s half-life. The original depth-1 map made with the ACT standard cuts algorithm can be seen in the left panel. The same depth-1 map but with features identified by the random forest to be a PS with high probability (>70%) included can be seen in the right panel. Note that the source was almost completely cut out using the original method, which could result in it not being detected during map-based searches, but using our modified pipeline, it was recovered.</em>
</p>
