...
from enlib import config, scanutils, log, coordinates, mapmaking, sampcut, cg, dmap, errors, bench
...
parser.add_argument("--tod_sim_file", type=str, default = '/home/eh9397/TOD_sims/240126-1121240126-1121_1607824359.1607834320.ar5:f090.npy')
...
def calc_b_new(self, tod_provider=None):
		...
		for scan in self.scans:
			...
			if tod_provider is None:
				with bench.mark("b_read"):
					# tod_old  = scan.get_samples(verbose=False)
					#read in sims
					tod = np.load(args_dict['tod_sim_file'])
					tod  = tod.astype(self.dtype)
					tod  = utils.deslope(tod)
					print(np.shape(tod), flush = True)
					# scan.ndet = np.shape(tod)[0]
			else:
